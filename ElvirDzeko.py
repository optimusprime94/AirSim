...
from cntk.logging import TensorBoardProgressWriter, time
...
def compute_reward(quad_state, quad_vel, collision_info,collision, goal_state, old_dist):
    ...
    pts = [np.array([-.55265, -31.9786, zt]), np.array([48.59735, -63.3286, zt]),
           np.array([193.5974, -55.0786, zt]), np.array([369.2474, 35.32137, zt]),
           np.array([541.3474, 143.6714, zt])]

    quad_pt = np.array(list((quad_state.x_val, quad_state.y_val, quad_state.z_val)))

    new_distance = np.sqrt(
        np.power((goal_state[0] - quad_state.x_val), 2) + np.power((goal_state[1] - quad_state.y_val), 2))
